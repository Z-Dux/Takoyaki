...
export default {
  ...
  async execute(interaction: CommandInteraction) {
    ...
    const zoro = new ANIME.Zoro(BASE);
    const search = await zoro.search(query);
    const result =
      search.results.find((x) => x.id.startsWith(query)) || search.results[0];
    ...
    if (anime.genres)
      str.push(`Genres: ${anime.genres.map((g) => g).join(", ")}`);
    ...
    if (anime.japaneseTitle.length > 0)
      embed.data.title += ` (${anime.japaneseTitle})`;
    ...
    if (anime.studios)
      str.push(`Studios: ${anime.studios.map((s) => s).join(", ")}`);
    if (description)
      embed.setDescription(description.toString().substring(0, 250) + `...`);
    ...
    embed.addFields([
      {
        ...
        value: str
          .map(
            (x) =>
              `- **${x.substring(0, x.indexOf(`:`) + 1)}**${x.substring(
                x.indexOf(`:`) + 1
              )}`
          )
          .join("\n"),
  ...
  async autocomplete(interaction: Interaction) {
    ...
    const zoro = new ANIME.Zoro(BASE); //(`http://consumet-rose-one.vercel.app/`)
    ...
    const choices = results.map((result) => ({
      name:
        typeof result.title === "string" ? result.title : result.title.english,
      value: result.id,
    }));
    const res = choices
      .map((choice) => ({
