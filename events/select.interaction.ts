...
export default {
  ...
  execute: async (interaction: Interaction, client: Client) => {
    if (interaction.isAnySelectMenu()) {
      const animeId = interaction.customId.split("-").slice(1, -1).join("-");
...
const urlBtn = (url: string) =>
  new ButtonBuilder().setEmoji("🔗").setURL(url).setStyle(ButtonStyle.Link);

function controlBtns(btns: ControlBtns[], id: string, url?: string) {
  // Play, Add to Queue
  const buttons = btns
    .map((btn) => {
